...
public class Node {
     ...
     *                           start
     */
    public Node(NodeType nodeType, String use, int port, int maxPeers, int initialConnections, int numNodes,
            int quorumSize, int minimumTransaction, int debugLevel) {
        ...
        synchronized (memPoolLock) {
            ...
            for (String hash : keys) {
                ...
                if (ptTransaction.getEvent().getAction().name().equals("Prescription")) {
                    ...
                    if (!foundAPatient)
                        System.out.println("Node: " + myAddress.getPort() + "(Doctor): never found a patient");
    ...
    public void sendSkeleton() {
        synchronized (lock) {
            ...
            for (String hash : quorumBlock.getTxList().keySet()) {
                ...
                if (!transaction.getEvent().getAction().name().equals("Algorithm"))
            ...
            try {
                ...
                skeleton = new BlockSkeleton(quorumBlock.getBlockId(),
                        ...
                        getBlockHash(quorumBlock, 0), vrs);
            } catch (NoSuchAlgorithmException e) {
                throw new RuntimeException(e);
            }
            ...
            if (USE.equals("Prescription")) {
                ...
                if (listOfVRS.size() > 0) {
                    for (ArrayList<ValidationResultSignature> vrsList : listOfVRS) {
                        PtTransaction ptTransaction = (PtTransaction) blockTransactions
                                .get(vrsList.get(0).getVr().getPtTransactionHash()); // Assuming there is at least one
                                                                                     // VR
    ...
    public void addBlock(Block block) {
        ...
        if (inQuorum()) {
            ...
            if (block.getBlockId() == 0) { // genesis block
                ...
                gossipTransaction(
                        new PtTransaction(new Algorithm(algorithmSeed), String.valueOf(System.currentTimeMillis())));
